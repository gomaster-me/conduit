...
func NewTestHelper() *TestHelper {
	...
	version := flag.String("conduit-version", "", "the version of conduit to test")
	namespace := flag.String("conduit-namespace", "conduit", "the namespace where conduit is install")
	...
	if *version == "" {
		exit(1, "-conduit-version flag is required")
	}
...
// GetVersion returns the version of conduit to test. Set the version using the
// -conduit-version command line flag.
...
func (h *TestHelper) ConduitRun(arg ...string) (string, error) {
	...
	return h.CombinedOutput("conduit", withNamespace...)
}
...
func (h *TestHelper) ConduitRunStream(arg ...string) (*Stream, error) {
	withNamespace := append(arg, "--conduit-namespace", h.namespace)
	cmd := exec.Command("conduit", withNamespace...)
...
// return the last error received from the function.
